...
map.defaults = {
    ...
    prompt_test_patterns = {
        ...
        "^%[.*%]",
        "^[Hh][Pp]:.*>",
        "^{HP:%d+/%d+ %d+%%|MP:%d+/%d+|Mv:%d+%%|AC:%-[%w]+|TNL:[%d,%-]+}$",
        "^{HP:%d+/%d+ %d+%%|MP:%-?%d+/%d+|Mv:%d+%%|AC:%-[%w]+|TNL:[%d,%-]+}(.+: %[%d+%%%])? ?<%(%%d+%%%%)> $"
    },
...
local function config()
    ...
    if map.configs.download_path ==
        "https://raw.githubusercontent.com/JorMox/Mudlet/development/src/mudlet-lua/lua/generic-mapper" then
    ...
    local protected = {"mapping", "currentRoom", "currentName", "currentExits", "currentArea", "prevRoom", "prevName",
                       "prevExits", "mode", "version"}
...
local function parse_help_text(text)
    ...
    text = text:gsub("%$ROOM_EXITS_STATUS",
        (not map.currentExits or table.is_empty(map.currentExits)) and '❌' or '✔️')
...
local bool_configs = {'stretch_map', 'search_on_look', 'speedwalk_wait', 'speedwalk_random', 'clear_lines_on_send',
                      'debug', 'custom_name_search', 'use_translation'}
...
local function move_map()
    ...
    if move or random_move then
        ...
        elseif move == map.configs.lang_dirs['look'] and map.currentRoom and
            not check_room(map.currentRoom, map.currentName, map.currentExits) then
            ...
            if exits[move] and (vision_fail or check_room(exits[move], map.currentName, map.currentExits, onlyName)) then
                set_room(exits[move])
            elseif special[move] and
                (vision_fail or check_room(special[move], map.currentName, map.currentExits, onlyName)) then
...
local function capture_move_cmd(dir, priority)
    ...
    local portal = string.match(dir, "^nav?i?g?a?t?e? (%a+)")
    if map.mapping and portal and portal ~= "snowglobe" then
        local portals = getSpecialExitsSwap(map.currentRoom)
        if not portals[dir] then
            map.set_portal(dir, true)
        end
    end
    if table.contains(exitmap, dir) or string.starts(dir, "nav?i?g?a?t?e? ") or dir == "recall" then
...
function map.set_exit(dir, roomID)
    ...
    if map.mapping then
        ...
        if not string.starts(dir, "-p ") then
            local exit
            if stubmap[exitmap[dir] or dir] <= 6 then
...
function map.find_path(roomName, areaName, return_tables)
    ...
    if return_tables then
        ...
        if #dirs > 0 then
            map.echo("Path to " .. roomName .. ((areaName and " in " .. areaName) or "") .. ": " ..
                         table.concat(dirs, ", "))
        else
...
function map.echoRoomList(areaname, exact)
    ...
    if areaid then
        ...
        cecho(format("<%s>List of all rooms in <%s>%s<%s> (areaID <%s>%s<%s> - <%s>%d<%s> rooms):\n", listcolor,
            othercolor, msg, listcolor, othercolor, areaid, listcolor, othercolor, #roomlist, listcolor))
...
function map.echoAreaList()
    ...
    for id = 1, table.maxn(rlist) do
        if rlist[id] then
            ...
            fg(listcolor)
            echoLink(string.format("%-40s (%d rooms)", rlist[id], countrooms(id)),
                'map.echoRoomList("' .. id .. '", true)', "View the room list for " .. rlist[id], true)
            echo("\n")
        end
    end
...
local function check_version()
    ...
    if pos ~= #versions then
        ...
        map.echo(string.format("The Generic Mapping Script is currently <red>%d<reset> versions behind.",
            #versions - pos))
...
function map.eventHandler(event, ...)
    ...
    elseif event == "sysDownloadError" and downloading then
        ...
        if string.ends(file, "/versions.lua") and mudlet.translations.interfacelanguage == "zh_CN" then
            ...
            if map.configs.download_path ==
                "https://raw.githubusercontent.com/Mudlet/Mudlet/development/src/mudlet-lua/lua/generic-mapper" then
                map.configs.download_path =
                    "https://gitee.com/mudlet/Mudlet/raw/development/src/mudlet-lua/lua/generic-mapper"
                map.checkVersion()
            end
        end
...
map.registeredEvents = {registerAnonymousEventHandler("sysDownloadDone", "map.eventHandler"),
...
function map.relativeroom(from, dir)
    if not map.roomexists(from) then
        return
    end
...
function map.roomFind(query, lines)
    ...
    if not tonumber(select(2, next(result))) then
        ...
        for _, v in ipairs(roomsTable) do
            ...
            roomname = v.name
            roomarea = v.area
            cechoLink(string.format("<cyan> %-10s", roomid), 'map.gotoRoom(' .. roomid .. ')',
                string.format("Go to %s (%s)", roomid, tostring(roomname)), true)
            cecho(string.format("<LightSlateGray>%-40s", string.sub(tostring(roomname), 1, 39)))
            cechoLink(string.format("<DarkSlateGrey>%s<DarkSlateGrey>\n", getRoomAreaName(getRoomArea(roomid))),
                [[map.echoPath(map.currentRoom, ]] .. roomid .. [[)]], "Display directions from here to " .. roomname,
                true)
            resetFormat()
            i = i + 1
        end
        ...
        for roomname, roomid in pairs(result) do
            ...
            cechoLink("<cyan>" .. roomid, 'map.gotoRoom(' .. roomid .. ')',
                string.format("Go to %s (%s)", roomid, tostring(roomname)), true)
            cecho(string.format("<DarkSlateGrey>) in <LightSlateGray>%s<DarkSlateGrey>.",
                getRoomAreaName(getRoomArea(roomid))))
            fg("DarkSlateGrey")
            echoLink(" > Show path\n", [[map.echoPath(map.currentRoom, ]] .. roomid .. [[)]],
                "Display directions from here to " .. roomname, true)
            resetFormat()
        end
    end
    if table.size(result) <= lines then
        ...
        lastRoomQuery = query
        cechoLink(string.format("<DarkSlateGrey>%d of %d rooms shown. Click to see all rooms.\n", lines,
            table.size(result)), 'map.roomFind(lastRoomQuery, "all")',
            string.format("Show all %d rooms.", table.size(result)), true)
    end
end
...
function map.lockArea(name, lock, dontreshow)
    ...
    if not dontreshow then
        map.doLockArea(map.lastLockSearch)
    end
end
...
function map.roomLook(input)
    ...
    local function handle_number(num)
        ...
        -- generate a report
        map.echo(string.format("Room: %s #: %d area: %s (%d)", name, num, getRoomAreaName(areanum), areanum))
        map.echo(string.format("Coordinates: x:%d, y:%d, z:%d, locked: %s, weight: %s", coords[1], coords[2], coords[3],
            (islocked and "yes" or "no"), tostring(weight)))
        map.echo(string.format("Environment: %s (%d)%s", tostring(envname), env,
        ...
        for exit, leadsto in pairs(exits) do
            echo(string.format("  %s -> %s (%d)%s%s\n", exit, getRoomName(leadsto), leadsto, ((getRoomArea(leadsto) or
                "?") == areanum and "" or " (in " .. (getRoomAreaName(getRoomArea(leadsto)) or "?") .. ")"),
                ((not exitweights[map.anytoshort(exit)] or exitweights[map.anytoshort(exit)] == 0) and "" or
                    " (weight: " .. exitweights[map.anytoshort(exit)] .. ")")))
        end
    ...
    local function handle_name(name)
        ...
        if not select(2, next(result)) or not tonumber(select(2, next(result))) then
            for roomid, roomname in pairs(result) do
                roomid = tonumber(roomid)
                cecho(string.format("  <LightSlateGray>%s<DarkSlateGrey> (", tostring(roomname)))
                cechoLink("<cyan>" .. roomid, 'map.roomLook(' .. roomid .. ')',
                    string.format("View room details for %s (%s)", roomid, tostring(roomname)), true)
...
function map.createMapFeature(featureName, roomCharacter)
    ...
    if type(roomCharacter) ~= "string" then
        map.echo("The new room character must be either a string or nil. " .. type(roomCharacter) .. " is not allowed.")
        return
    end
    ...
    if not mapFeatures[lowerFeatureName] then
        ...
        map.echo("Created map feature '" .. featureName .. "' with the room character '" .. roomCharacter .. "'.")
    else
...
function map.roomCreateMapFeature(featureName, roomId)
    ...
    local mapFeatures = loadMapFeatures()
    if not mapFeatures[lowerFeatureName] then
        map.echo("A feature with name '" .. featureName .. "' does not exist. You need to use 'feature create' first.")
        return
    end
...
function map.deleteMapFeature(featureName)
    ...
    for _, roomId in pairs(roomsWithFeature) do
        ...
        if not deletionResult then
            map.echo("Something went wrong deleting the map feature '" .. featureName ..
...
function map.echoPath(from, to)
    ...
    if getPath(from, to) then
        map.echo("<white>Directions from <yellow>" .. string.upper(searchRoom(from)) .. " <white>to <yellow>" ..
                     string.upper(searchRoom(to)) .. "<white>:")
        map.echo(table.concat(speedWalkDir, ", "))
        return map.speedWalkDir
    else
        map.echo("<white>I can't find a way from <yellow>" .. string.upper(searchRoom(from)) .. " <white>to <yellow>" ..
                     string.upper(searchRoom(to)) .. "<white>")
    end
end
...
function map.listSpecialExits(filter)
                    ...
                    if not filter or cmd:lower():find(filter, 1, true) then
                        if getRoomArea(exit) ~= area then
                            cecho(string.format(
                                ...
                                getRoomName(rooms[i]), rooms[i], areaname, cmd, getRoomName(exit), exit,
    ...
    map.echo(string.format("%d exits listed%s.", c, (not filter and '' or ", with for the filter '" .. filter .. "'")))
end
...
function map.delSpecialExits(filter)
    ...
    map.echo(string.format("%d exits deleted%s.", c, (not filter and '' or ", with for the filter '" .. filter .. "'")))
end
...
function map.doareadelete(areaid)
    ...
    function map.deletenextbatch()
        ...
        map.echo(string.format("Deleted %d batch%s so far, %d left to go - %.2f%% done out of %d needed",
...
function map.renameArea(name, exact)
    if not (map.currentroom or getRoomArea(map.currentRoom)) then
        ...
        map.echo(string.format("Renamed %s to %s (%d).", getRoomAreaName(getRoomArea(map.currentRoom)), name,
...
function map.roomArea(otherroom, name, exact)
    ...
    if id then
        ...
        map.echo(string.format("Moved %s to %s (%d).",
            (getRoomName(otherroom) ~= "" and getRoomName(otherroom) or "''"), fname, id))
    ...
    elseif next(ma) then
        ...
        for _, name in ipairs(ma) do
            echo("  ")
            setUnderline(true)
            echoLink(name, [[map.roomArea('', "]] .. name .. [[", true)]], "Move the room to " .. name, true)
            setUnderline(false)
            echo("\n")
        end
        resetFormat()
...
function map.roomLabel(input)
    if not createMapLabel then
        map.echo("Your Mudlet doesn't support createMapLabel() yet - please update to 2.0-test3 or better.")
        return
    end
    ...
    map.echo(string.format("Created new label #%d '%s' in %s.", lid, message, getRoomAreaName(getRoomArea(room))))
end

function map.areaLabels(where, exact)
    if not getMapLabels then
        map.echo("Your Mudlet doesn't support getMapLabels() yet - please update to 2.0-test3 or better.")
        return
    end
    ...
    if areaid then
        ...
        for labelid, labeltext in pairs(t) do
            ...
            fg("orange_red")
            setUnderline(true)
            echoLink('delete', string.format('deleteMapLabel(%d, %d); map.echo("Deleted label #' .. labelid .. '")',
                areaid, labelid), "Delete label #" .. labelid .. " from " .. getRoomAreaName(areaid))
            setUnderline(false)
            echo(")\n")
        end
        resetFormat()
    elseif not areaid and #multiples > 0 then
        ...
        for _, areaname in ipairs(multiples) do
            echo("  ");
            setUnderline(true)
            echoLink(areaname, 'map.areaLabels("' .. areaname .. '", true)', "Click to view labels in " .. areaname,
                true)
            setUnderline(false)
            echo("\n")
        end
